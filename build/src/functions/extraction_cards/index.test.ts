...
describe('extraction_cards function', () => {
  // Test utilities
  ...
  it('should return success when cards extraction completes successfully', async () => {
  ...
  it('should return false when board ID is missing in event context', async () => {
    const mockEvent = createMockEvent();
  ...
  it('should return error details when spawn throws an error', async () => {
    ...
    expect(result).toEqual({
      success: false,
      message: 'Error during cards extraction: Test spawn error',
      details: testError
    });
  });
