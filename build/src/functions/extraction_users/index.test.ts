...
describe('extraction_users function', () => {
  // Test utilities
  ...
  it('should return success when users extraction completes successfully', async () => {
  ...
  it('should return false when organization ID is missing in connection data', async () => {
    const mockEvent = createMockEvent();
  ...
  it('should return error details when spawn throws an error', async () => {
    ...
    expect(result).toEqual({
      success: false,
      message: 'Error during users extraction: Test spawn error',
      details: testError
    });
  });
