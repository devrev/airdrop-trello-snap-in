...
import { setupTest, createMockEvent, createMockPayload } from './test-helpers';

describe('extraction_external_sync_unit_check function', () => {
  // Test utilities
  ...
  it('should return success when external sync units extraction completes successfully', async () => {
  ...
  it('should return false for non-external sync units extraction event types', async () => {
    ...
    expect(result).toEqual({
      success: false,
      message: `Event type ${EventType.ExtractionMetadataStart} is not an external sync units extraction event`
    });
  ...
  it('should return error details when spawn throws an error', async () => {
    ...
    expect(result).toEqual({
      success: false,
      message: 'Error during external sync units extraction: Test spawn error',
      details: testError
    });
  });
