...
describe('push_boards_as_sync_units function', () => {
  // Test utilities
  ...
  it('should return success when pushing boards as sync units completes successfully', async () => {
    // Mock successful spawn execution that fetches boards and their card counts
  ...
  it('should return error details when spawn throws an error', async () => {
    ...
    expect(result).toEqual({
      success: false,
      message: 'Error pushing boards as external sync units: Test spawn error',
      details: testError
    });
  });
