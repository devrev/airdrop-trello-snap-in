...
export async function sendEventToSnapIn(event: any): Promise<any> {
  try {
    ...
    if (axios.isAxiosError(error)) {
      throw new Error(`Failed to send event to snap-in server: ${error.response?.status} ${error.response?.statusText} - ${JSON.stringify(error.response?.data)}`);
    }
    throw new Error(`Failed to send event to snap-in server: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}

export function cleanupCallbackServer(setup: CallbackServerSetup): Promise<void> {
