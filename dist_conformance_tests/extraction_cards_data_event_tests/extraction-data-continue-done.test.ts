...
describe('Extraction Function - Data Continue to Done Validation', () => {
  ...
  function loadAndPrepareTestEvent(): any {
    try {
      ...
      const testEventPath = path.join(__dirname, 'data_extraction_continue_test.json');
  ...
  test('should receive EXTRACTION_DATA_DONE event from EXTRACTION_DATA_CONTINUE with cards artifact having item_count=150', async () => {
    // Load and prepare the test event
    const event = loadAndPrepareTestEvent();
    
    // Verify this is indeed an EXTRACTION_DATA_CONTINUE event
    expect(event.payload.event_type).toBe('EXTRACTION_DATA_CONTINUE');
    ...
    if (artifactArray.length === 0) {
      throw new Error('Artifacts array is empty. Expected at least one artifact (len(artifact_array) > 0).');
    }
    ...
    if (!cardsArtifact) {
      ...
      throw new Error(
        `No cards artifact found. Expected one element with item_type='cards'. ` +
        `Available item types: [${availableItemTypes.join(', ')}]. ` +
    ...
    console.log(`✓ Successfully validated cards artifact with item_count=${itemCount} from EXTRACTION_DATA_CONTINUE event`);
  }, 120000);
});