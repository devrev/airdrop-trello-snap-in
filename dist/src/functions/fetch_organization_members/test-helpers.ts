...
export function createMockEvent(overrides: Partial<FunctionInput> = {}): FunctionInput {
  ...
  if (overrides.payload) {
    ...
    // Handle connection_data separately to allow complete replacement
    if (overrides.payload.connection_data !== undefined) {
      mergedEvent.payload.connection_data = overrides.payload.connection_data;
    } else if (mergedEvent.payload.connection_data) {
      // Only merge if both exist
      mergedEvent.payload.connection_data = {
        ...baseEvent.payload.connection_data,
        ...(overrides.payload.connection_data || {})
      };
    }
  }
...
export function setupSuccessfulMembersFetch() {
  ...
  // Create mock getOrganizationMembers function
...
export function setupFailedMembersFetch(statusCode: number, message: string, apiDelay = 0, headers = {}) {
  // Mock parseApiCredentials
  mockedParseApiCredentials.mockReturnValue({
    apiKey: 'test-api-key',
    token: 'test-token'
  ...
  // Create mock getOrganizationMembers function that returns error response
  const mockGetOrganizationMembers = jest.fn().mockResolvedValue({
...
export function setupRateLimitMock(retryAfter = '60') {
  return setupFailedMembersFetch(
    429,
