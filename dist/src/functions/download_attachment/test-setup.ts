...
export const createMockInputData = (
  idCard: string = 'test-card-id',
  idAttachment: string = 'test-attachment-id',
...
export const createMockEvent = (
  ...
  input_data: createMockInputData(idCard, idAttachment, fileName),
});
...
// Assertion helpers
export const expectSuccessResponse = (result: DownloadAttachmentResponse, expectedData?: any) => {
  ...
  if (expectedData) {
    expect(result.file_data).toBe(expectedData.file_data);
    expect(result.file_name).toBe(expectedData.file_name);
    expect(result.content_type).toBe(expectedData.content_type);
  }
};
